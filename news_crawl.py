...
while currentDate <= endDate:
    ...
    # 검색 쿼리 url
    base_url = (
        "http://news.naver.com/main/search/search.nhn?query=%s&st=news.all&q_enc=EUC-KR&r_enc=UTF-8&r_format=xml&rp=none&sm=%s.basic&ic=all&so=rel.dsc&detail=1&pd=1&r_cluster2_start=1&r_cluster2_display=10&start=1&display=10&startDate=%s&endDate=%s&page="
    ...
    while True:
        ...
        print(
            "Date: %d-%02d-%02d page: %d, totalCrawledNum: %d"
            % (currentYear, currentMonth, currentDay, page, articleNum)
        )
        page += 1
    ...
    if len(articleList) != 0:
        with open(f"{args.dir}/{currentYear}_{currentMonth:02d}_{currentDay:02d}.json", "w") as f:
